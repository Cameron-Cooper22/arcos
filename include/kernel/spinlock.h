#ifndef __ARCOS_SPINLOCK
#define __ARCOS_SPINLOCK
...
void spin_try_lock(volatile spinlock_t* lock);
...
static inline void read_spin_lock(volatile rwspinlock_t* lock) {
  ...
  if (readers == 1) spin_lock(&lock -> wr_lock);
  preempt_inc();
...
static inline void read_spin_unlock(volatile rwspinlock_t *lock) {
  ...
  if (readers == 0) spin_unlock(&lock -> wr_lock);
  preempt_dec();
  spin_unlock(&lock -> rd_lock);
}
